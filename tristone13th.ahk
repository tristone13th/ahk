...
{
    StringSplit, longName, a_loopfield, -
    name := Trim(longName1, " ")
    StringReplace, name, name, %A_Space%, _, All
    StringLen len, name

    If (!HasVal(tasklist, name)) {
        tasklist.Push(name)
    }
}
...
RAlt & c::
    global taskmap
    code := taskmap["Visual_Studio_Code"]
...
JEE_ExpGetTaskbarItems(vSep="`n")
{
    ...
    ; why choose 4? refer this https://www.autohotkey.com/boards/viewtopic.php?f=5&t=40526
